...
from .serializers import RecipeSerializer, IngredientSerializer, CartSerializer, \
	TagSerializer, FavoritesSerializer, RecipeCreateSerializer, \
...
class RecipeViewSet(viewsets.ModelViewSet):
	...
	def download_shopping_cart(self, request):
		...
		pdfmetrics.registerFont(
			TTFont(font, "data/Roboto-Regular.ttf", "UTF-8")
		)
...
class UserCustomViewSet(UserViewSet):
	...
	def delete_subscribe(self, request, id = None):
		user = self.request.user
		author = get_object_or_404(User, pk = id)
		instance = Follow.objects.filter(user = user,
										 author = author)
